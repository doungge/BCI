...
class _Transition(nn.Sequential):
    def __init__(self, num_input_features, num_output_features, 
        ...
        self.add_module("convt", nn.Conv1d(num_input_features, num_output_features, kernel_size=1, stride=1, bias=bias))
        self.add_module("actt", snn.Leaky(beta=0.9, spike_grad=spike_grad, init_hidden=True))
        self.add_module("poolt", nn.AvgPool1d(kernel_size=2, stride=2))
...
def spiking_ETRI(num_init_channels, norm_layer: callable = None, single_step_neuron: callable = None, **kwargs) -> SpikingDenseNet:
    ...
    Args:
        ...
        norm_layer (callable): a layer of batch norm. No batch norm if None
    """
    return _densenet("densenet121", 1, (2, 3, 8, 4), num_init_channels, norm_layer, single_step_neuron, **kwargs)



class ConNet(nn.Module):
  def __init__(self, in_c = 2, mid_c=8, out_c=15):
    super().__init__()
    num_filters = mid_c
    self.layers = nn.Sequential(
    nn.Conv1d(in_c, 2*num_filters, kernel_size=7, padding=3, padding_mode="zeros", stride=3),
    nn.ReLU(),
    nn.Conv1d(2*num_filters, num_filters, kernel_size=5, padding=2, padding_mode="zeros", stride=2),
    nn.ReLU(),
    nn.Flatten(),
    nn.Linear(32*num_filters, 128),
    nn.ReLU6(),
    nn.Linear(128, 64),
    nn.ReLU6(),
    nn.Linear(64, out_c, bias=False),
    nn.Softmax(dim=1)
    )
  def forward(self, x):
    print(x.size())
