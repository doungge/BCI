...
class DSCONV2(nn.Module):
    def __init__(self, dim):
        super().__init__()        
        self.dim = dim
        self.proj_conv = nn.Conv1d(4, 8, kernel_size=7, stride=3, padding=3, padding_mode="zeros", bias=True)
...
class spike_BCRNet(nn.Module):
    def __init__(self,
                 in_channels=2, num_classes=15, dim = 16, T=4          
                 ):
...
class spike_BCRNet_S(nn.Module):
    def __init__(self,
                 in_channels=2, num_classes=15, dim = 8, T=4                 
                 ):
...
for T in t: 
    ...
    for fold, (train_ids, test_ids) in enumerate(kfold.split(dataset)):
        ...
        if args.model == 'BCRNet':
            model = spike_BCRNet(2,8,15,T) 
        elif args.model == 'BCRNet_S':
            model  = spike_BCRNet_S(2,8,15,T)
        model.to(device)
        ...
        for epoch in range(n_epochs):
            ...
            if valid_loss < min_loss : 
                ...
                torch.save(model.state_dict(), f'./trained_model/{args.model}/model_direct_ll{T}_fold{fold}.pth')
            ...
            with open(f'./result/{args.model}/loss_direct_ll{T}_fold'+str(fold)+'.csv', 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerows([TL,VL])
            with open(f'./result/{args.model}/acc_direct_ll{T}_fold'+str(fold)+'.csv', 'w', newline='') as f:
                writer = csv.writer(f)
